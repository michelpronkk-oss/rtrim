      ...
      {runs.length === 0 ? (
        <div className="rounded-xl border border-white/7 bg-[#0C0C20] px-6 py-12 text-center">
          ...
          <p className="mx-auto mt-2 max-w-[440px] text-[13px] leading-[1.7] text-[#5E6A88]">
            Every synced run will include the prompt, scoped contract, memory used, risk score, token savings, and continuation pack. Sync opens for early access plans.
          </p>
          <Link
        ...
        <div className="overflow-hidden rounded-xl border border-white/7">
          <div className="hidden grid-cols-[2fr_1fr_auto_auto_auto_auto_auto] gap-4 border-b border-white/7 bg-[#0C0C20] px-5 py-3 md:grid">
          ...
          {runs.map((run, i) => (
            <Link
              ...
              className={`flex flex-col gap-2.5 px-5 py-4 transition-colors hover:bg-white/[0.025] md:grid md:grid-cols-[2fr_1fr_auto_auto_auto_auto_auto] md:items-center md:gap-4 ${
                i < runs.length - 1 ? "border-b border-white/6" : ""
              }`}
              style={{ background: i % 2 === 0 ? "#0C0C20" : "#0A0A1C" }}
            >
              <p className="truncate text-[13px] font-medium text-[#EDEEFF]">
                {run.task ?? "Untitled run"}
              </p>
              <p className="font-mono text-[12px] text-[#4D5070]">
                {run.project_id ? (projectMap[run.project_id] ?? "—") : "—"}
              </p>
              <Badge label={run.status} kind="status" />
              <Badge label={run.risk_after ?? run.risk_before} kind="risk" />
              <p className="font-mono text-[12px] text-[#9699BE]">
