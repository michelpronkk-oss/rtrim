...
const HOMEPAGE_FAQS = [
  ...
  },
  {
    ...
    a: "No. Restore is local recovery. You can recover without spending another agent run.",
  },
  {
    q: "What happens when a run is BLOCKED?",
    a: "BLOCKED means needs review. The run is recorded as evidence, but is not trusted yet.",
  },
  {
  ...
  {
    q: "What is included in Free vs Pro vs Builder?",
    a: "Free is local-first for one project. Pro adds cloud sync and history. Builder adds unlimited projects and CI Gate.",
  },
  {
    q: "Does RunTrim replace Git?",
    a: "No. Git remains your version history. RunTrim adds control, verification and recovery for agent runs.",
  },
];
...
export default function Home() {
  return (
    <div style={{ background: "#08090b", minHeight: "100vh", overflowX: "hidden", color: "#c9ccd2", fontFeatureSettings: '"ss01","ss02","cv11"' }}>
      ...
      >
        <div className="mx-auto" style={{ maxWidth: 1240, padding: "0 clamp(20px,4vw,40px)" }}>
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-[320px_1fr] lg:gap-12">
            <div>
              <p
                style={{
            ...
            <div className="space-y-3">
              {HOMEPAGE_FAQS.map((item) => (
                <article
                  key={item.q}
                  className="rounded-[9px] border border-white/8 bg-[#0c0e11] p-4"
                >
                  <h3 className="text-[14px] font-semibold text-[#f4f5f7]">{item.q}</h3>
                  <p className="mt-2 text-[13px] leading-[1.65] text-[#8a8f98]">{item.a}</p>
                </article>
              ))}
              <div className="pt-1">
                <Link
